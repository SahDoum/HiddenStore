...
async def cmd_order(message: types.Message):
    ...
    order = await HiddenOrder.create(items=items, price=100, user=user)  # Adjust price and items as needed
    await message.reply(MESSAGES['order_success'].format(order_id=order.order.id))
    # except Exception as e:
    #     logger.error(f"Error creating order for user id: {telegram_id}, error: {e}")
